...
- **模型输出不可信**：Markdown 经 DOMPurify 净化，Mermaid 使用 strict 模式；外部链接只交给系统浏览器，应用窗口禁止远程导航
- **命令执行**：execFile 命令/参数分离，无 shell 注入面；白名单精确匹配
- **文件访问**：工作区/信任目录内免弹窗，之外弹窗授权（写操作权限与读一致）
- **设置白名单**：IPC 只接受应用主页面请求，并执行预定义键 + 类型校验
- **密钥安全**：设置中的 API Key 使用系统安全存储加密，且不会返回渲染进程；支持环境变量/设置/~/.dsh 三级来源
- **网络边界**：远程 API Base 必须使用 HTTPS，仅 localhost 兼容端点允许 HTTP
